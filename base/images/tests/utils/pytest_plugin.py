...
def pytest_configure(config) -> None:  # type: ignore[no-untyped-def]
    ...
    config.addinivalue_line(
        ...
        "used for -m filtering to separate static and runtime suites",
    )
    config.addinivalue_line(
        "markers",
...
def pytest_collection_modifyitems(config, items) -> None:  # type: ignore[no-untyped-def]
    ...
    - ``@pytest.mark.runtime_container_tests`` on any test under
      ``cases/runtime/`` so static-only suites can exclude them with
    ...
    for item in items:
        ...
        # Auto-apply runtime_container_tests marker.
        if category == "runtime":
